...
export function Interactive3DGlobe() {
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      const grad = ctx.createRadialGradient(centerX, centerY, globeRadius * 0.7, centerX, centerY, globeRadius * 1.25)
      grad.addColorStop(0, 'rgba(61, 255, 160, 0.03)')
      grad.addColorStop(0.5, 'rgba(95, 185, 255, 0.04)')
      ...
      for (let i = 0; i < dots.length; i++) {
        ...
        if (normalizedZ > 0.08) {
          const alpha = Math.max(0.04, normalizedZ * 0.6)
          const dotSize = Math.max(0.8, normalizedZ * 1.8)

          ctx.fillStyle = `rgba(146, 165, 160, ${alpha})`
          ctx.beginPath()
      ...
      const hub = projectedNodes[0] // Tel Aviv
      for (let i = 1; i < projectedNodes.length; i++) {
        ...
        if (hub.pz > -globeRadius * 0.4 || target.pz > -globeRadius * 0.4) {
          ...
          ctx.strokeStyle = `rgba(255, 75, 62, ${arcAlpha})`
          ctx.lineWidth = 1.2
          ctx.setLineDash([3, 5])
          ctx.stroke()
          ctx.setLineDash([])
      ...
      projectedNodes.forEach((node) => {
        if (node.pz > -globeRadius * 0.2) {
          ...
          const beaconSize = isCurrentSelected ? 4.5 : 3
          const pulse = (Math.sin(pulseTime * 3) + 1) / 2

          ctx.beginPath()
          ctx.arc(node.px, node.py, beaconSize + pulse * 6, 0, Math.PI * 2)
          ...
          if (node.pz > 0 || isCurrentSelected) {
            ...
            ctx.fillStyle = isCurrentSelected ? '#e6efec' : '#92a5a0'
  ...
  const handleMouseMove = (e: React.MouseEvent) => {
    ...
    stateRef.current.targetRotationY += dx * 0.008
    stateRef.current.targetRotationX = Math.max(-0.9, Math.min(0.9, stateRef.current.targetRotationX - dy * 0.008))
  ...
  return (
    <div className="relative flex flex-col items-center justify-center rounded-2xl border border-line bg-bg-panel p-6 shadow-[0_20px_60px_-15px_rgba(0,0,0,0.8)]">
      <div className="absolute top-4 left-4 flex items-center gap-2 font-mono text-[11px] text-text-faint">
        <span className="h-2 w-2 animate-blip rounded-full bg-red" />
        <span>OSN GLOBAL THREAT GRID // LIVE SIMULATION</span>
      </div>

      <div className="absolute top-4 right-4 flex items-center gap-1.5 font-mono text-[10px] text-text-dim">
        <span className="border border-line px-2 py-0.5">DRAG 3D SPHERE</span>
      </div>
      ...
      <div className="mt-2 w-full border-t border-line pt-4">
            ...
            return (
              <button
                ...
                className={`rounded border px-2.5 py-1 font-mono text-[11px] transition-all ${
                  isSel
                    ? 'border-red bg-red/15 text-text font-bold shadow-[0_0_12px_rgba(255,75,62,0.3)]'
        ...
        <div className="mt-3 flex items-center justify-between rounded border border-line-soft bg-bg-panel-alt p-3 font-mono text-xs">
          <div>
          ...
          <div className="text-right">
            ...
            <span className="text-[10px] text-text-faint">INTERCEPTED EVENTS</span>
          </div>
        </div>
      </div>
    </div>
